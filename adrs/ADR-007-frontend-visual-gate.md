...
> out on Colossus. Vite bound `localhost`, which on that dual-stack machine is `[::1]:5173` and
> nothing else (measured with `ss -lntp`), while Playwright polled `127.0.0.1`; and
> `webServer` discarded the child's output so the failure reported no
