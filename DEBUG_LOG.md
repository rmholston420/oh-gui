...
## 2026-08-09 03:48 EDT — Attainability mutation tests compared floating-point values exactly

- **Symptom:**  and  in .
- **Affected stage / plugin / port:** Phase 0 parallel benchmark track · tool-call attainability gate.
- **Root cause:** Binary floating-point representation differed by insignificant rounding at the final bit.
- **Fix applied:** Replaced exact float equality with ; mutation tests pass.
- **Files changed:**
- **Related BUILD_LOG entry:** staged in

## 2026-08-09 03:49 EDT — Correction to the preceding attainability-log entry

- **Symptom:** The preceding debug-log entry lost literal backticked test text because shell command substitution expanded it while appending.
- **Affected stage / plugin / port:** project log maintenance.
- **Root cause:** The log append used an unquoted heredoc delimiter around Markdown backticks.
- **Fix applied:** This append uses a quoted heredoc delimiter. The substantive diagnosis is: `bench/toolcall/tests/test_attainability.py` compared `5.101020514433646` with `5.101020514433644` and `0.719999999999999` with `0.72` using exact equality; binary floating-point rounding caused both failures. Replaced those assertions with `math.isclose(..., abs_tol=1e-12)`.
- **Files changed:** `DEBUG_LOG.md`, `bench/toolcall/tests/test_attainability.py`
- **Related BUILD_LOG entry:** staged in `/tmp/blog-bench.md`

## 2026-08-09 03:52 EDT — Tool-call task provenance tripped the evidence-snapshot guard

- **Symptom:** `npm run gate` failed the `evidence_snapshot_not_imported` and `evidence_snapshot_matches_upstream` constraints after task fixtures recorded local evidence-snapshot paths.
- **Affected stage / plugin / port:** Phase 0 parallel benchmark track · tool-call task fixtures.
- **Root cause:** The hard-constraint guard forbids runtime and benchmark files from referencing the vendored SDK snapshot. A one-off Python import attempt also wrote untracked bytecode into that immutable snapshot.
- **Fix applied:** Reused the recorded snapshot discipline: removed only generated bytecode and replaced fixture provenance with the OpenHands Tools 1.41.0 package paths and upstream URL, not the local snapshot location. The final GUI gate passes.
- **Files changed:** `bench/toolcall/tasks/*.json`, `bench/toolcall/tests/test_tasks.py`, `DEBUG_LOG.md`
- **Related BUILD_LOG entry:** staged in `/tmp/blog-bench.md`

## 2026-08-09 05:26 EDT — TS2591 `Cannot find name 'node:fs'` blocks the whole gate

- **Symptom:** `src/shell/breakpoint-arithmetic.test.ts(9,30): error TS2591: Cannot find name
  'node:fs'. Do you need to install type definitions for node?` (also `node:url`, `node:path`).
  `npm run gate` fails at the build step even though Vitest passes.
- **Affected stage / plugin / port:** Phase 1 · GUI tooling · no port
- **Root cause:** `tsconfig.app.json` includes `src` whole and sets `"types": ["vite/client"]` on
  purpose, so that browser code importing Node builtins fails to typecheck. A test placed beside
  its source inherits that restriction.
- **Fix applied:** put Node-using tests in `src/__tests__/`, which `tsconfig.app.json` excludes and
  `tsconfig.node.json` checks with `"types": ["node", ...]`. Do not add `@types/node` to the app
  project — that would remove the browser-purity guarantee entirely.
- **Two traps found while fixing it:** (1) `tsconfig.app.json` had a duplicate `exclude` key, so an
  added exclusion was silently dropped; JSON keeps the last occurrence and reports nothing.
  (2) Substituting Vite's `import CSS from "./Shell.css?raw"` typechecks and runs, but the test
  runner stubs CSS imports to `''`, so all seven `toContain` assertions pass vacuously. Prefer a
  build error over a green test that asserts nothing.
